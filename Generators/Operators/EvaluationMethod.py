...
class evalDiscriminator():
    """
    Discriminator-based evaluation method. Using a pre-trained Wasserstein GAN
    discriminator, it evaluates the reconstructed population and estimates how
    real each is.

    Attributes:
        model: A predictive model that takes a tensor of sequences and returns a
               tensor of predictions.
    """
    def __init__(self, model):
        self.model = model
    
    def __call__(self, masked_sequence, reconstructed_population, **kwargs):
        
        input_tensor = np.stack([masked_sequence] * len(reconstructed_population))
        ga_tensor = np.stack(reconstructed_population)

        # Stack into tensors
        input_tensor = torch.tensor(np.stack(input_tensor), dtype=torch.float32)
        ga_tensor = torch.tensor(np.stack(reconstructed_population), dtype=torch.float32)

        # Combine and format
        combo_tensor = torch.cat((input_tensor, ga_tensor), dim=2)
        combo_tensor = combo_tensor.permute(0, 2, 1)
        predictions = self.model(combo_tensor).squeeze()

        return predictions.cpu().detach().numpy().tolist()
