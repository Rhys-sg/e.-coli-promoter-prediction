...
class MogaLineage:
    '''
    A lineage of the Multi-Objective Genetic Algorithm (MOGA).

    It has a few differences from the Lineage class:
    - The MOGA lineage stores the each fitness of the best individual.
    - The MOGA lineage does not assume a cnn, so it does not have a final prediction.
    - Elitism does not use tools.selBest, and can be defined by the user.

    '''
    def __init__(self, toolbox, population_size, crossover_rate, mutation_prob, reconstruct_sequence, reverse_one_hot_sequence, elitism_rate, survival_rate, track_history):
    ...
    def run(self, generations=1):
        ...
        for _ in range(generations):
            ...
            elite = self.toolbox.select_elite(self.population, elite_size)
            ...
            current_best = self.toolbox.select(self.population, 1)[0]
    ...
    def _update_best(self, individual):
        if self.best_sequence is None or self.best_fitness is None:
            ...
            best_individual = self.toolbox.clone(individual)
            best_individual.fitness.values = self.best_fitness
            selected = self.toolbox.select([best_individual, individual], 1)[0]
